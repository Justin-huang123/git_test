...
static bool avoidModeFlag = false;
...
static void flight(void)
{
	...
	if(avoidModeFlag == true)
    {
        oled_showChar(14,30,'A',12,24,1);
    }
    else
    {
        // 原本的逻辑
	...
	else if(keyState == KEY_J1_LONG_PRESS)
        {
            // 安全起见：只有在解锁状态下(RCLock == false)且连上飞机时，才允许切模式
            if(RCLock == false && radioinkConnectStatus() == true)
            {
                avoidModeFlag = !avoidModeFlag; // 翻转模式状态
                
                // 发送控制指令 0x09，并把当前模式状态发过去（1代表避障，0代表遥控）
                sendRmotorCmd(CMD_MODE_SWITCH, avoidModeFlag ? 1 : 0);
                
                // 调用 beep.c 里的动作，发出一声短促的滴声提示操作成功
                runBeepAcktion(TRIM_BEEP); 
            }
        }
	/*事件处理*/
